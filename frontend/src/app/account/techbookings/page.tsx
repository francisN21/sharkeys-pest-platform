...
import { getAdminTechBookings, reassignBooking, type TechRow, type TechBookingRow } from "../../../lib/api/adminTechBookings";
...
export default function TechBookingsPage() {
  ...
  if (loading) {
    return <div className="p-4 text-sm" style={{ color: "rgb(var(--muted))" }}>Loading…</div>;
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="space-y-4">
        {technicians.map((t) => {
          const list: TechBookingRow[] = t.bookings ?? [];

          return (
            <section key={t.user_id} className="rounded-2xl border p-4 space-y-3" style={{ borderColor: "rgb(var(--border))" }}>
              ...
              {list.length === 0 ? (
                ...
                <div className="grid gap-3">
                  {list.map((b) => (
                    <div key={b.public_id} className="rounded-2xl border p-4 space-y-2" style={{ borderColor: "rgb(var(--border))" }}>
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
      ...
      {reassignOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border p-5 space-y-4" style={{ borderColor: "rgb(var(--border))", background: "rgb(var(--card))" }}>
            <div className="flex items-start justify-between gap-3">
              <div>
