...
const LOCKED_SERVICE_TITLE = "Commercial Services";
...
export default function NewCustomerBookingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const pestExtermination =
          list.find((s) => s.title.trim().toLowerCase() === LOCKED_SERVICE_TITLE.toLowerCase()) ||
          null;

        if (!pestExtermination) {
          setError(`"${LOCKED_SERVICE_TITLE}" service was not found.`);
          setServicePublicId("");
          return;
        }

        setServicePublicId(pestExtermination.public_id);
      } catch (e: unknown) {
        if (!alive) return;
  ...
  function resetForm() {
    ...
    const pestExtermination =
      services.find((s) => s.title.trim().toLowerCase() === LOCKED_SERVICE_TITLE.toLowerCase()) ||
      null;
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!servicePublicId) return setError("Pest Extermination service is unavailable.");
...
<section className="mx-auto max-w-5xl space-y-6 px-4 py-10">
  <div className="space-y-4">
    <div>
      <Link
        href="/#services"
        className="inline-flex items-center gap-2 rounded-xl border px-4 py-2 text-sm font-semibold hover:opacity-90"
        style={{
    ...
    <div className="space-y-1">
      <h1 className="text-2xl font-semibold">Book Commercial Services</h1>
      <p className="text-sm" style={{ color: "rgb(var(--muted))" }}>
        New customer booking for Commercial Services. Choose your date and time, then submit your request.
      </p>
    </div>
  </div>
            ...
            <form className="space-y-6" onSubmit={onSubmit}>
              ...
              <div className="space-y-2">
                ...
                {selectedService ? (
                  ...
                  >
                    Pest Extermination service is unavailable.
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-end gap-3">
                ...
                <button
                  ...
                  style={{ borderColor: "rgb(var(--border))", background: "rgb(var(--card))" }}
                  disabled={loadingSubmit || selectedStartHour === null || !servicePublicId}
                >
